...
The first step to approach this problem is mining the locations of every restaurant. Luckily for us, Zomato has a free [API](https://developers.zomato.com/api) that is very rich and easy to use (but not without its restrictions). In order to geographically constrain the analysis I focused only on the locations within the city limits of Lisbon (which I found [here](http://geodados.cm-lisboa.pt/datasets/freguesias-2012)). This decreased the total number of shops down to **45**. Once we have these two very simple datasets we can visualize their distribution.
...
Thus, if we compute the red-shaded area and divide it by the total city area we get a nice round value of **25%.**
